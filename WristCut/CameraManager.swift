...
class CameraManager: NSObject, ObservableObject {
    ...
    func startSession() async {
        ...
        if WCSession.default.activationState == .activated && WCSession.default.isReachable {
            WCSession.default.sendMessage(["event": "session_started"], replyHandler: nil)
        }
        ...
        if !savedCellularWatch {
            let delay = savedStartDelayNanos
            let totalSeconds = Int(Double(delay) / 1_000_000_000)
            if totalSeconds <= 0 {
                ...
                Task {
                    for remaining in stride(from: totalSeconds, through: 1, by: -1) {
                        if remaining > 60 {
                            statusMessage = "Starting in \(remaining / 60) min..."
                        } else {
                            statusMessage = "Starting in \(remaining)s..."
                        }
